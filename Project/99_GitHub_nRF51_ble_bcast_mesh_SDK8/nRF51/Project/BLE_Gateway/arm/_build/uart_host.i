...
void uart_init(void)
{
    ...
    do { uint16_t APP_UART_UID = 0; app_uart_buffers_t buffers; static uint8_t rx_buf[256]; static uint8_t tx_buf[256]; buffers . rx_buf = rx_buf; buffers . rx_buf_size = sizeof (rx_buf); buffers . tx_buf = tx_buf; buffers . tx_buf_size = sizeof (tx_buf); err_code = app_uart_init(&comm_params, &buffers, uart_event_handle, APP_IRQ_PRIORITY_LOW, &APP_UART_UID); } while (0);
...
void Uart_data_write(uint8_t * p_data, uint16_t length)
{
    ...
    {
        while(app_uart_put(p_data[i]) != ((0x0) + 0));
    }
}
