...
#line 37 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 39 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 40 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 41 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 42 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 43 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 44 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 45 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 46 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
#line 47 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
#line 1 "..\\..\\..\\..\\..\\..\\components\\softdevice\\s110\\headers\\nrf_soc.h"
...
#line 48 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 49 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
#line 50 "..\\..\\..\\rbc_mesh\\src\\transport_control.c"
...
static uint8_t tx_data[((28) + ((3) + (6))) * (1)];
...
static inline void packet_create_from_data(uint8_t* data, packet_t* packet)
{
    ...
    packet->length = (data[(1)] & (0x3F)) - (6);
    memcpy(packet->data, &data[((3) + (6))], packet->length);
    memcpy(packet->sender.addr, &data[(3)], (6));

     
    _Bool addr_is_random = (data[(0)] & (0x40));

    if (addr_is_random)
    {
        _Bool is_static = ((packet->sender.addr[5] & 0xC0) == 0xC0);
        if (is_static)
        {
            packet->sender.addr_type = 0x01;
        }
        else
        {
            _Bool is_resolvable = ((packet->sender.addr[5] & 0xC0) == 0x40);
...
static inline _Bool packet_is_data_packet(uint8_t* data)
{
    return ((data[(0)] & (0x0F))
        == (0x02));
}
...
static void search_callback(uint8_t* data)
{
    ...
    uint32_t checksum = (((NRF_RADIO_Type *) 0x40001000UL)->RXCRC & 0x00FFFFFF);

     
    uint64_t radio_time_left = timeslot_get_remaining_time();

    if (radio_time_left > (500))
    {
    ...
    if (data == 0 || !packet_is_data_packet(data))
    {
        return;
    }
...
static void trickle_step_callback(void)
{
    ...
    if (timeslot_get_remaining_time() < (500))
        return;
    ...
    packet_t packet;
    _Bool has_anything_to_send = 0;

    mesh_srv_packet_assemble(&packet, (28) * (1),
    ...
    if (has_anything_to_send)
    {
        ...
        uint8_t packet_and_addr_type = (0x02) |
            ((packet.sender.addr_type == 0x00)?
            0 :
            (0x40));
        ...
        tx_data_ptr[(0)] = packet_and_addr_type;

        
 
        do
        {
            uint8_t min_len = ((packet.length > (28))?
                (28) :
                packet.length);

            tx_data_ptr[(2)] = 0;
            tx_data_ptr[(1)] = (min_len + (6));
            tx_data_ptr[(0)] = packet_and_addr_type;

            memcpy(&tx_data_ptr[(3)], packet.sender.addr, (6));
            memcpy(&tx_data_ptr[((3) + (6))], &temp_data_ptr[0], min_len);
    ...
    if (error_code == ((0x0) + 0) && next_time < global_time + end_time)
    {
