...
uint32_t sd_evt_handler(void)
{
    ...
    while (sd_ble_evt_get((uint8_t*) &ble_evt, &len) == ((0x0) + 0))
    {
...
{   
    ...
    ((NRF_POWER_Type *) 0x40000000UL)->RESET = 1;
    
    Relay_Init();
     
    gpio_init();
     
    do { static uint32_t BLE_EVT_BUFFER[(((((sizeof(ble_evt_t) + (23))) - 1) / (sizeof(uint32_t))) + 1)]; uint32_t ERR_CODE; ERR_CODE = softdevice_handler_init((NRF_CLOCK_LFCLKSRC_XTAL_75_PPM), BLE_EVT_BUFFER, sizeof(BLE_EVT_BUFFER), sd_evt_handler); do { const uint32_t LOCAL_ERR_CODE = (ERR_CODE); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 0, 0); } while (0); } } while (0); } while (0);
...
#line 232 "..\\main.c"
    ...
    ((NRF_GPIO_Type *) 0x50000000UL)->OUTCLR = (1 << 4);
...
void Input_Cyclic_Process( void )
{
	...
	uint8_t Loc_var;
	uint8_t RFTx_buf[4] = {'#','W','2', 0};
	
	Loc_var = (uint8_t)nrf_gpio_pin_read(16);
